...
const bankEs = require("./data/lider-bank.json");
const bankEn = require("./data/lider-bank-en.json");
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const check = validateCodigoEval(codigo, "LIDERA", participante);
    if (!check.ok) {
      return res.status(403).json({
    ...
    return res.status(200).json({
      ok: true,
      version: bank.version || (lang === "EN" ? "LID-EN-v1" : "LID-v1"),
      n: items.length,
    ...
    console.error("banco-liderazgo", err);
