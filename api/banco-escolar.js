...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const check = validateCodigoEval(codigo, "SOCIES", participante);
    if (!check.ok) {
      return res.status(403).json({
    ...
    const items = (bank.items || []).map((it) => {
      ...
      return { id: it.id, pregunta: it.preg };
    });

    return res.status(200).json({
      ok: true,
      version: bank.version || "SOCIESC-v1-banco-107",
      n: items.length,
    ...
    console.error("banco-escolar", err);
